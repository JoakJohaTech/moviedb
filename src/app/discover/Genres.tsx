...
const Genres = (props: Props) => {
  ...
  return(
    <div className="flex flex-col justify-center items-center">
      <div className="md:flex md:flex-row md:flex-wrap grid grid-cols-3 gap-4 cursor-pointer">
        {data.genres?.map((item: MovieGenre, index: number) => {
          if(ApprovedGenres.includes(item.name)) {
            return(
              <div className="p-1 z-10" key={index}>
                <span className="border-solid border-2 rounded-3xl w-28 h-9 focus-within:bg-white p-2 focus-within:text-black">
                  <input
        ...
        <div className="flex flex-col justify-center items-center pt-10">
          <div className="flex">
            {discover?.map((item: MovieList, index: number) => {
              return(
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 w-full h-full cursor-pointer" key={index}>
